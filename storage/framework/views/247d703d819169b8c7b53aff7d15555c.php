...
<?php $__env->startSection('title','Timesheet Management'); ?>
<?php $__env->startSection('page-heading','Timesheet Management'); ?>
    ...
    <table class="min-w-full text-sm">
      ...
      <tbody class="divide-y divide-gray-100">
        <?php $__currentLoopData = $rows; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $r): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
          <tr>
            <td class="px-4 py-2"><input type="checkbox" class="chkRow" value="<?php echo e($r['id']); ?>"></td>
            <td class="px-4 py-2">
              <div class="font-medium text-gray-900"><?php echo e($r['user']); ?></div>
              <div class="text-xs text-gray-500">#U<?php echo e(str_pad($r['user_id'], 3, '0', STR_PAD_LEFT)); ?></div>
            </td>
            <td class="px-4 py-2">
              <div class="font-medium text-gray-900">
                <?php echo e($r['week_start']->format('d M')); ?> – <?php echo e($r['week_end']->format('d M Y')); ?>

              </div>
              <div class="text-xs text-gray-500">W<?php echo e($r['week_start']->format('W')); ?></div>
            </td>
            <td class="px-4 py-2 text-gray-700"><?php echo e($r['site']); ?></td>
            <td class="px-4 py-2 text-gray-600"><?php echo e($r['submitted_at']->format('d M Y, h:i A')); ?></td>
            <td class="px-4 py-2 font-medium"><?php echo e(number_format($r['total'], 2)); ?></td>
            <td class="px-4 py-2">
            ...
            <td class="px-4 py-2 text-right">
              <div class="inline-flex items-center gap-1">
                <button class="rounded-md border px-2 py-1 hover:bg-gray-50" data-review="<?php echo e($r['id']); ?>">Review</button>
                <button class="rounded-md bg-green-600 text-white px-2 py-1 hover:bg-green-700" data-approve="<?php echo e($r['id']); ?>">Approve</button>
                <button class="rounded-md bg-red-600 text-white px-2 py-1 hover:bg-red-700" data-reject="<?php echo e($r['id']); ?>">Reject</button>
                <button class="rounded-md bg-amber-600 text-white px-2 py-1 hover:bg-amber-700" data-modify="<?php echo e($r['id']); ?>">Modify</button>
              </div>
            </td>
          </tr>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
      ...
      <form class="p-4 space-y-4" action="#" method="POST">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          <div>
            ...
            <input type="text" class="w-full rounded-lg border border-gray-300 px-3 py-2" value="<?php echo e($weekStart->format('d M')); ?> – <?php echo e($weekStart->copy()->endOfWeek()->format('d M Y')); ?>" disabled>
          </div>
          <div>
            <label class="block text-sm text-gray-600 mb-1">Status</label>
...
<script>
  ...
  document.getElementById('btnExportTS')?.addEventListener('click', () => {
    const rows = <?php echo json_encode($rows->map(fn($r)=>[
      $r['id'], $r['user'], $r['week_start']->format('Y-m-d')) ?>;
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\laragon\www\srtaff-portal\resources\views/admin/timesheets.blade.php ENDPATH**/ ?>