...
<?php $__env->startSection('sidebar'); ?>
    ...
    <a href="<?php echo e(route('admin.jobs-applications')); ?>" class="block px-3 py-2 rounded-lg hover:bg-gray-50">💼 Jobs &
        Applications</a>
...
<?php $__env->startSection('content'); ?>
                ...
                <tbody class="divide-y divide-gray-100">
                    <?php $__currentLoopData = $jobs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $job): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <tr>
                            ...
                            <td class="px-4 py-2">
                                <span
                                    class="inline-flex items-center rounded-md px-2 py-1 text-xs font-medium ring-1 ring-inset <?php echo e($statusBadge($job['status'])); ?>">
                ...
                <tbody class="divide-y divide-gray-100">
                    <?php $__currentLoopData = $applications; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $a): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <tr>
                            ...
                            <td class="px-4 py-2 text-right">
                                <div class="inline-flex items-center gap-1">
                                    <a href="<?php echo e($a['cv_url']); ?>" class="rounded-md border px-2 py-1 hover:bg-gray-50">View
                                        CV</a>
                                    <button class="rounded-md border px-2 py-1 hover:bg-gray-50"
                                        data-view="<?php echo e($a['id']); ?>">View</button>
                                    <button
                                        class="rounded-md <?php echo e($a['shortlisted'] ? 'bg-amber-600 text-white hover:bg-amber-700' : 'bg-brand-600 text-white hover:bg-brand-700'); ?> px-2 py-1"
                                        data-shortlist="<?php echo e($a['id']); ?>">
                                        <?php echo e($a['shortlisted'] ? 'Unshortlist' : 'Shortlist'); ?>

                                    </button>
                                    <button class="rounded-md border px-2 py-1 hover:bg-gray-50"
                                        data-export-one="<?php echo e($a['id']); ?>">Export</button>
                                    <button class="rounded-md bg-red-600 text-white px-2 py-1 hover:bg-red-700"
                                        data-delete-app="<?php echo e($a['id']); ?>">Delete</button>
                                </div>
                            </td>
                        </tr>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
  ...
  const rows = <?php echo json_encode($jobRows, 15, 512) ?>;
...
if (el.id === 'btnExportCandidates') {
  const rows = <?php echo json_encode($candidateRows, 15, 512) ?>;
...
if (el.id === 'btnExportShortlisted') {
  const rows = <?php echo json_encode($shortlistedRows, 15, 512) ?>;
  const csv = `id,job_id,job_title,name,email,phone,experience,submitted_at,shortlisted\n` +
    rows.map(row => row.map(v => `"${String(v).replaceAll('"', '""')}"`).join(',')).join('\n');
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/u461551961/domains/wponline.io/public_html/staff_portal/resources/views/admin/jobs-applications.blade.php ENDPATH**/ ?>