...
@section('page-heading','Payroll Management')
    ...
    <form action="#" method="POST" enctype="multipart/form-data" class="p-5 space-y-4" id="payslipForm">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <div>
          ...
          <input type="date" name="issued" value="{{ now()->format('Y-m-d') }}" class="w-full rounded-lg border border-gray-300 px-3 py-2 focus:ring-2 focus:ring-brand-500" required>
        </div>
      </div>
          ...
          @foreach($payslips as $p)
            <tr data-id="{{ $p['id'] }}">
              <td class="px-4 py-2">
                ...
                <div class="text-xs text-gray-500">#U{{ str_pad($p['user_id'],3,'0',STR_PAD_LEFT) }}</div>
              </td>
              <td class="px-4 py-2">{{ $p['period'] }}</td>
              <td class="px-4 py-2 text-gray-600">{{ $p['issued']->format('d M Y') }}</td>
              <td class="px-4 py-2">{{ $currency($p['gross']) }}</td>
              <td class="px-4 py-2">{{ $currency($p['deductions']) }}</td>
              <td class="px-4 py-2 font-medium">{{ $currency($p['net']) }}</td>
              <td class="px-4 py-2 text-right">
                <div class="inline-flex items-center gap-1">
                  <a href="{{ $p['url'] }}" class="rounded-md border px-2 py-1 hover:bg-gray-50">View</a>
                  <a href="{{ $p['url'] }}" class="rounded-md bg-brand-600 text-white px-2 py-1 hover:bg-brand-700">Download</a>
...
{{-- ================= Helper: Sample CSV for Bulk Upload (optional UI hint) ================= --}}
