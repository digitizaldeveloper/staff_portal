...
@section('title','My Profile & Timesheets')
@section('page-heading','My Profile & Timesheets')
...
<div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
  ...
  <section class="lg:col-span-1 bg-white border border-gray-200 rounded-xl p-5">
    <div class="flex items-center gap-4">
      <img src="{{ $user->avatar_url ?? 'https://i.pravatar.cc/80' }}" alt="Avatar"
           class="h-16 w-16 rounded-full ring-2 ring-brand-100">
      <div>
        <h2 class="text-lg font-semibold text-gray-900">{{ $user->name }}</h2>
        <p class="text-sm text-gray-600">
          Role: <span class="font-medium">{{ $user->role->name ?? 'Staff' }}</span>
        </p>
        <p class="text-xs text-gray-500">Employee ID: <span class="font-mono">{{ $user->employee_code ?? $user->id }}</span></p>
      </div>
    </div>
    ...
    <div class="mt-5 grid grid-cols-2 gap-3 text-sm">
      ...
      <div class="rounded-lg bg-gray-50 p-3">
        ...
        <p class="font-medium">{{ optional($user->created_at)->format('d M Y') }}</p>
      </div>
      <div class="rounded-lg bg-gray-50 p-3">
        <p class="text-gray-500">Status</p>
        <span class="inline-flex items-center rounded-md px-2 py-1 text-xs font-medium
          {{ $user->active ?? true ? 'bg-green-50 text-green-700 ring-1 ring-inset ring-green-600/20' : 'bg-red-50 text-red-700 ring-1 ring-inset ring-red-600/20' }}">
          ...
          <li class="flex items-start justify-between rounded-lg border border-gray-200 p-3">
            <div>
              ...
              <p class="text-xs text-gray-500">
                Role: {{ $as['role'] ?? '—' }}
                @if(!empty($as['since']))
                  • Since: {{ \Illuminate\Support\Carbon::parse($as['since'])->format('d M Y') }}
                @endif
              </p>
            </div>
    ...
    <div class="p-5">
      <div class="mb-3 flex items-center gap-2">
        <form method="GET" class="flex items-center gap-2">
          <input type="text" name="q" value="{{ request('q') }}" placeholder="Search week/date…"
          ...
          <select name="status" class="rounded-lg border border-gray-300 px-3 py-2 text-sm focus:ring-2 focus:ring-brand-500">
            ...
            @foreach(['draft'=>'Draft','submitted'=>'Submitted','approved'=>'Approved','rejected'=>'Rejected'] as $k=>$v)
            ...
            @forelse($timesheets as $ts)
              <tr>
                <td class="px-4 py-2 font-medium text-gray-900">
                  {{ \Illuminate\Support\Carbon::parse($ts->week_start)->format('d M') }}
                  – {{ \Illuminate\Support\Carbon::parse($ts->week_end)->format('d M Y') }}
                </td>
                <td class="px-4 py-2 text-gray-600">
                  {{ $ts->submitted_at ? \Illuminate\Support\Carbon::parse($ts->submitted_at)->format('d M Y, h:i A') : '—' }}
                </td>
                <td class="px-4 py-2">{{ number_format($ts->total_hours, 2) }}</td>
                <td class="px-4 py-2">
                  @php
                    $badge = [
