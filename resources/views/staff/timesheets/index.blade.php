@extends('layouts.app')
@section('page-heading', 'My Timesheets')
                ...
                <tbody class="divide-y divide-gray-100 bg-white">
                    @forelse ($timesheets as $ts)
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            <td class="px-6 py-4">
                                <div class="font-semibold text-gray-900">{{ \Carbon\Carbon::parse($ts->date)->format('d M Y') }}</div>
                                <div class="text-xs text-gray-400">#{{ str_pad($ts->id, 4, '0', STR_PAD_LEFT) }}</div>
                            </td>
                            <td class="px-6 py-4 text-gray-900">{{ $ts->client->name }}</td>
                            <td class="px-6 py-4 font-semibold text-gray-900">{{ $ts->total_hours }} hrs</td>
                            <td class="px-6 py-4 text-gray-500">{{ $ts->admin_notes ?: '—' }}</td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 text-right">
                                @if ($ts->status == 'pending')
                                    <div class="inline-flex items-center gap-2">
                                        <a href="{{ route('staff.timesheets.edit', $ts->id) }}"
                               ...
                               <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                         ...
                                         <button onclick="openModal({{ $ts->id }})"
                                        type="button"
