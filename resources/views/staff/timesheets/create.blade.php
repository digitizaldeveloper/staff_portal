@extends('layouts.app')
@section('page-heading', 'Create Timesheet')
...
<div class="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
    ...
    <form method="POST" action="{{ route('staff.timesheets.store') }}" class="space-y-5 px-6 py-6">
        @csrf
        ...
        <div class="space-y-1">
            ...
            <textarea name="notes" id="notes" rows="4"
                      class="w-full rounded-2xl border border-gray-200 px-4 py-2.5 text-gray-900 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                      placeholder="Optional context for reviewer">{{ old('notes') }}</textarea>
        </div>
